function test_file_io(; n1=6, n2=8, n3=12)
    ...
    mktempdir_parallel() do p
        ...
        for T=(Float32, Float64), ns=(:C, :I)
            ...
            grid = ABL.Grid((n1, n2, n3))
            pdims = ABL.PhysicalSpace.default_size(grid)
            transform = ABL.PhysicalSpace.Transform2D(T, pdims)
            ...
            fvel = ABL.PhysicalSpace.set_field!(vel0, zeros(T, grid, NS(ns)),
                                                transform, domain, grid, NS(ns))
            vel = ABL.PhysicalSpace.get_field(transform, fvel)

            for T_files=(Float32, Float64)

                fn = joinpath(p, string("vel-", T_files, "-from-", T, "-", ns, ".cbd"))
                ABL.CBD.writecbd(T_files, fn, vel, x1, x2, x3local, xmin, xmax, grid.comm)

                xmin_after, xmax_after, x1_after, x2_after, x3_after, vel_after =
                    ABL.CBD.readcbd(fn, x3local, grid.comm)
...
function test_shifted_file_output(T=Float64; n1=6, n2=8, n3=12)
    ...
    for ns = (:C, :I)
        ...
        fvel = ABL.PhysicalSpace.set_field!(vel0, zeros(T, grid, NS(ns)),
                                            transform, domain, grid, NS(ns))
        vel = ABL.PhysicalSpace.get_field(transform, fvel, centered = true)
        ...
        xmin_file, xmax_file, x1_file, x2_file, x3_file, data_file = mktempdir_parallel() do p
            ...
            ABL.CBD.writecbd(T, fn, vel, x1, x2, x3local, xmin, xmax, grid.comm)
            ABL.CBD.readcbd(fn, x3local, grid.comm)
        end
