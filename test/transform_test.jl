function test_transform_constant()
    ...
    domain = ABL.Domain(ds, nothing, nothing)
    ABL.PhysicalSpace.set_field!((x,y,z)->1, uh, ht, domain, gd, NS(:C))
    ABL.PhysicalSpace.set_field!((x,y,z)->2, uv, ht, domain, gd, NS(:I))
    ...
    uh_pd = ABL.State.get_field(ht, uh)
    uv_pd = ABL.State.get_field(ht, uv)
    npd = ABL.PhysicalSpace.default_size(gd)
...
function test_transform_horizontally_varying()
    ...
    ABL.PhysicalSpace.set_field!(u0, uh, ht, domain, gd, NS(:C))
...
function test_transform_vertically_varying()
    ...
    ABL.PhysicalSpace.set_field!((x,y,z)->z, uh, ht, domain, gd, NS(:C))
    ABL.PhysicalSpace.set_field!((x,y,z)->z, uv, ht, domain, gd, NS(:I))
    ...
    uh_pd = ABL.State.get_field(ht, uh)
    uv_pd = ABL.State.get_field(ht, uv)
    n1pd, n2pd = ABL.PhysicalSpace.default_size(gd)
    @test global_vector(uh_pd[1,1,:]) ≈ global_values_h
    @test global_vector(uv_pd[1,1,:]) ≈ global_values_v
    @test uh_pd ≈ uh_pd[1:1,1:1,:] .* ones(ABL.PhysicalSpace.default_size(gd)..., gd.n3c)
    @test uv_pd ≈ uv_pd[1:1,1:1,:] .* ones(ABL.PhysicalSpace.default_size(gd)..., gd.n3i)
end
